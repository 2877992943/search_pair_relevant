...
def str_stem(s): 
	...
	if isinstance(s,unicode):
		try:
			s=s.encode()#unicode->other encoding method   |decode :other encodingMethod->unicode way
			s=str_stem(s)
        		return s
		except:
			print '1',chardet.detect(s)['encoding'],s
			s=s.decode(chardet.detect(s)['encoding'])
			print '1',chardet.detect(s)['encoding'],s
			return 'null'#str_stem(s)
			
			#print type(s),s
	else:
		print '2',chardet.detect(s)['encoding'],s
		s=s.decode(chardet.detect(s)['encoding']),s
		print '2',chardet.detect(s)['encoding']
		return 'null'#str_stem(s)

def seg_words(str1, str2):
    #print 'seg_words()...'
    str2 = str2.lower();#print str2
    str2 = re.sub("[^a-z0-9./]"," ", str2);#print str2
    str2 = [z for z in set(str2.split()) if len(z)>2];#print str2
    words = str1.lower().split(" ");#print words
    s = []
    for word in words:
        if len(word)>3:
            s1 = []
            s1 += segmentit(word,str2,True);#print word,s1
            if len(s)>1:
...
def segmentit(s, txt_arr, t):
    ...
    st = s
    r = []
    for j in range(len(s)):#each char_position in search_word
        for word in txt_arr: #each word in title or description
            if word == s[:-j]:
                r.append(s[:-j])
                #print(s[:-j],s[len(s)-j:])
                s=s[len(s)-j:]
...
if __name__=='__main__':
	...
	df_all = pd.concat((df_train, df_test), axis=0, ignore_index=True);#print df_all.values.shape
	...
	#print 'original...',df_all['search_term'],df_all['product_title']
	#df_all['search_term'] = df_all['search_term'].map(lambda x:str_stem(x));#print df_all['search_term']
	 
	df_all['product_title'] = df_all['product_title'].map(lambda x:str_stem(x));#print df_all['product_title']
	"""
	df_all['product_description'] = df_all['product_description'].map(lambda x:str_stem(x));#print df_all['product_description']
	df_all['brand'] = df_all['brand'].map(lambda x:str_stem(x));#print df_all['brand'] 
	print("--- Stemming: %s minutes ---" % round(((time.time() - start_time)/60),2))
	df_all['product_info'] = df_all['search_term']+"\t"+df_all['product_title'] +"\t"+df_all['product_description'];
	#print df_all['product_info']
	...
	#print 'seg words...',df_all['search_term']
