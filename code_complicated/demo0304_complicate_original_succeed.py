...
def getWidDic(brand,widDic):#array,dic
	for line in brand[:]:
		#print (line)
		line=line.split(' ')
		for wid in line:
			if wid not in widDic:
				widDic[wid]=len(widDic)
	#print (widDic)
	return widDic
...
def str2int(brandStr,widDic):#array  dic
	brandIntList=[]
	for line in brandStr[:]:
		#print (line)
		brandI=[]
		line=line.split(' ')
		for wid in line:
			brandI.append(widDic[wid])
		brandIntList.append(brandI)
	return brandIntList
...
df_all = pd.read_csv('../df_all1.csv', encoding="ISO-8859-1", index_col=0)
...
df_outliers = pd.read_csv('df_all1.csv', encoding="ISO-8859-1", index_col=0)
...
widDic={}
for att in ['product_title','search_term','product_description', 'brand']:
#for att in ['product_title']:#total 2w wid for 20w pair
#for att in ['product_description']:

	#att='product_description'
	arr=df_all[att].values;print (arr.shape)
	widDic=getWidDic(arr,widDic)

save2pickle(widDic,'widDic')
 
widDic=load_pickle('widDic')
print (len(widDic))

 


outPutfile=open(dataPath+'out.txt','w')
for wid in widDic.keys(): 
...
for att in ['product_info','product_title','search_term','product_description', 'brand'][:1]:
	brandStr=df_all[att].values[:];
	brandStr=np.array([brand.replace('\t',' ') for brand in brandStr]);#print (brandStr) #['simpson strong tie' 'simpson strong tie'...]
	#brandIntList=str2int(brandStr,widDic);print (brandIntList) #array['angle metl',...]   {str:ind....}->[[doc],[1,3,32]..]
	tfidf = TfidfVectorizer(ngram_range=(1, 1), stop_words='english')
	tsvd = TruncatedSVD(n_components=10, random_state = 2016)
	termDocMatrix=tfidf.fit_transform(brandStr)#.toarray();print (termDocMatrix.shape)#[n,d]
	#print (termDocMatrix.toarray(),termDocMatrix.toarray().shape)
	tdMatrix=tsvd.fit_transform(termDocMatrix);print (tdMatrix.shape)#[n,d]
	save2pickle(tdMatrix,'tdMatrix_'+att)#memory error
"""
...
"""
Index(['Unnamed: 0.1', 'id', 'product_title', 'product_uid', 'relevance',
